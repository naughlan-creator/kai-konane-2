...
`web` generates the id and forwards it as `X-Request-ID`; `api` reuses whatever
it is given and generates one only when called directly. So a single id spans
the whole hop, and Log Analytics can group by it.
